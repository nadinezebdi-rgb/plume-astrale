...
const PersonForm = ({ person, onChange, label, num }) => (
  <div className="space-y-4">
    <h3 className="text-lg flex items-center gap-2" style={{ fontFamily: 'Cormorant Garamond, serif', color: '#F5EEE0' }}>
      <Heart className="w-5 h-5 text-[#D4AF37]" strokeWidth={1.5} /> {label}
    </h3>

    <div className="grid grid-cols-2 gap-3">
      <div>
        <label className="block text-[#D4AF37] text-xs uppercase tracking-widest mb-1">Pr&eacute;nom</label>
        <input type="text" value={person.first_name} onChange={(e) => onChange('first_name', e.target.value)}
          className="w-full px-3 py-2 bg-[#15112A] border border-[#D4AF37]/30 rounded-lg text-[#B8B0C8] placeholder:text-[#B8B0C8]/30 focus:outline-none focus:border-[#D4AF37] text-sm"
          placeholder="Pr&#233;nom" data-testid={`person${num}-first-name`} />
      </div>
      <div>
        <label className="block text-[#D4AF37] text-xs uppercase tracking-widest mb-1">Nom</label>
    ...
    <div>
      <label className="block text-[#D4AF37] text-xs uppercase tracking-widest mb-1">Genre</label>
      <div className="flex gap-3">
        {[{ val: 'male', label: 'Homme' }, { val: 'female', label: 'Femme' }].map(g => (
          <button key={g.val} type="button" onClick={() => onChange('gender', g.val)}
            className={`flex-1 py-2 rounded-lg border text-sm transition-all ${person.gender === g.val ? 'border-[#D4AF37] bg-[#D4AF37]/20 text-[#F5EEE0]' : 'border-[#D4AF37]/20 text-[#B8B0C8]/60 hover:border-[#D4AF37]/40'}`}
    ...
    <div className="grid grid-cols-3 gap-3">
      <div>
        ...
        <input type="number" min="1" max="31" value={person.day} onChange={(e) => onChange('day', e.target.value)}
          className="w-full px-3 py-2 bg-[#15112A] border border-[#D4AF37]/30 rounded-lg text-[#B8B0C8] focus:outline-none focus:border-[#D4AF37] text-sm"
      ...
      <div>
        <label className="block text-[#D4AF37] text-xs uppercase tracking-widest mb-1">Ann&eacute;e</label>
    ...
    <div className="grid grid-cols-2 gap-3">
      <div>
        <label className="block text-[#D4AF37] text-xs uppercase tracking-widest mb-1">Heure</label>
        <input type="number" min="0" max="23" value={person.hour} onChange={(e) => onChange('hour', e.target.value)}
          className="w-full px-3 py-2 bg-[#15112A] border border-[#D4AF37]/30 rounded-lg text-[#B8B0C8] focus:outline-none focus:border-[#D4AF37] text-sm"
          data-testid={`person${num}-hour`} />
      </div>
      <div>
        <label className="block text-[#D4AF37] text-xs uppercase tracking-widest mb-1">Minute</label>
    ...
    <div>
      <label className="block text-[#D4AF37] text-xs uppercase tracking-widest mb-1">Lieu de naissance</label>
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="text-center mb-10">
            ...
            <div className="flex justify-center mb-6">
              <div className="relative w-48 h-48 md:w-64 md:h-64">
                ...
                <div className="absolute inset-0 rounded-full" style={{ background: 'radial-gradient(circle, transparent 60%, #111625 100%)' }} />
              </div>
            </div>
          ...
          <div className="flex items-center justify-center gap-2 mb-8">
            {[1, 2, 3, 4].map(s => (
              <div key={s} className="flex items-center gap-2">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-all ${step >= s ? 'bg-[#D4AF37] text-[#111625]' : 'border border-[#D4AF37]/30 text-[#D4AF37]/40'}`}>
                  {s}
                </div>
                {s < 4 && <div className={`w-8 h-0.5 ${step > s ? 'bg-[#D4AF37]' : 'bg-[#D4AF37]/20'}`} />}
              </div>
            ))}
          </div>
          )}
            ...
            <div className="animate-fade-in">
              {!isAuthenticated ? (
                ...
                <div className="flex flex-col items-center justify-center py-12 text-center" data-testid="credit-gate-insufficient">
                  ...
                  <p className="text-sm mb-2" style={{ color: 'var(--pa-muted)' }}>Ce rapport coûte <span style={{ color: '#D4AF37', fontWeight: 600 }}>10 crédits</span>.</p>
          ...
          {step === 3 && (
            <div className="card-mystical animate-fade-in" data-testid="step-3-question">
              ...
              <h3 className="text-lg flex items-center gap-2 mb-4" style={{ fontFamily: 'Cormorant Garamond, serif', color: '#F5EEE0' }}>
                <MessageCircle className="w-5 h-5 text-[#D4AF37]" strokeWidth={1.5} /> Votre Question
              </h3>
              ...
              <textarea
                ...
                rows={3}
                className="w-full px-4 py-3 bg-[#15112A] border border-[#D4AF37]/30 rounded-lg text-[#B8B0C8] placeholder:text-[#B8B0C8]/30 focus:outline-none focus:border-[#D4AF37] text-sm resize-none"
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {questionExamples.map((q, i) => (
                  <button key={i} onClick={() => setQuestion(q)}
                    className="text-xs px-3 py-1.5 rounded-full border border-[#D4AF37]/20 text-[#D4AF37]/70 hover:border-[#D4AF37]/50 hover:text-[#D4AF37] transition-all"
          ...
          {step === 4 && pdfUrl && (
            <div className="space-y-6 animate-fade-in" data-testid="step-4-result">
              <div className="card-mystical text-center p-10 glow-gold">
                ...
                <h2 className="text-2xl mb-3" style={{ fontFamily: 'Cormorant Garamond, serif', color: '#F5EEE0' }}>
                  Votre Rapport est Pr&ecirc;t !
                </h2>
                <p className="text-[#B8B0C8]/60 mb-2">
                  Analyse de compatibilit&eacute; entre <span className="text-[#F5EEE0] font-medium">{person1.first_name}</span> et <span className="text-[#F5EEE0] font-medium">{person2.first_name}</span>
                </p>
                <p className="text-[#D4AF37]/60 text-sm mb-6">Rapport d&eacute;taill&eacute; avec profils individuels, analyse des &eacute;l&eacute;ments, r&eacute;solution de conflits et cl&eacute;s de r&eacute;ussite</p>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <button onClick={() => navigate('/formulaire')} className="card-mystical hover:border-[#D4AF37]/50 transition-all group text-left" data-testid="upsell-manuscrit">
                  <div className="flex items-center justify-between">
                    <div>
                      <h3 className="text-[#F5EEE0] font-medium mb-1">Th&egrave;me Astral Pro</h3>
              ...
              <button onClick={() => { goToStep(1); setPdfUrl(null); setQuestion(''); }}
                className="text-[#D4AF37]/50 hover:text-[#D4AF37] text-sm mx-auto block transition-colors" data-testid="new-test-btn">
