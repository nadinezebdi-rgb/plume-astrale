...
import { CheckCircle2, Sun, Star, Moon, Loader2, Mail, Download } from 'lucide-react';
...
const TrioDecouverteSucces = () => {
  ...
  const poll = useCallback(async () => {
    if (!sessionId) return;
    try {
      const r = await axios.get(`${API}/api/trio-decouverte/status?session_id=${sessionId}`);
  ...
  return (
    <div className="min-h-screen relative" style={{ padding: '110px 20px 140px' }} data-testid="trio-decouverte-success-page">
      <SEO path="/trio-decouverte/succes" title="Ton Trio Découverte arrive · Plume Astrale" />

      <div className="max-w-2xl mx-auto text-center">
        <div className="mb-8" data-testid="trio-decouverte-success-icon">
          ...
          <h1 style={{ fontFamily: 'Cormorant Garamond, serif', fontWeight: 300, fontSize: 'clamp(32px, 5vw, 48px)', color: '#F5EEE0', lineHeight: 1.15 }}>
            Ton <em style={{ color: '#D4AF37', fontStyle: 'italic' }}>Trio</em> se compose
          </h1>
          <p className="mt-4" style={{ color: 'rgba(227,215,255,0.75)', fontFamily: 'Cormorant Garamond, serif', fontStyle: 'italic' }}>
            Trois portraits sacrés sont en cours de rédaction — livraison par email dans quelques minutes.
          </p>
        </div>

        {/* 3 PDF steps */}
        <div className="plume-glass p-6 md:p-8 mb-8" data-testid="trio-decouverte-steps">
          {['theme_natal', 'numerologie', 'kabbale_arbre_de_vie'].map((kind) => {
            const child = (status.children || []).find((c) => c.kind === kind);
            const meta = CHILD_META[kind];
            const Icon = meta.icon;
            const ready = child?.pdf_ready;
            return (
              <div key={kind} className="flex items-center gap-3 py-3" data-testid={`trio-child-${kind}`}>
                ...
                <div className="flex-1 flex items-center justify-between">
                  ...
                  {ready && child.pdf_url && (
                    <a href={child.pdf_url}
                      ...
                      data-testid={`trio-download-${kind}`}
        ...
        {!status.all_ready && (
          <p className="text-xs" style={{ color: 'rgba(227,215,255,0.55)', letterSpacing: '0.15em' }}>
            ...
            Tes 3 PDFs arriveront aussi par email dans les 5 minutes.
          </p>
        )}

        <div className="mt-8">
          <Link to="/" className="text-xs uppercase" style={{ color: 'rgba(212,175,55,0.7)', letterSpacing: '0.2em', textDecoration: 'none' }} data-testid="trio-decouverte-back-home">
            Retour à l&apos;accueil
          </Link>
        </div>
      </div>
...
export default TrioDecouverteSucces;
