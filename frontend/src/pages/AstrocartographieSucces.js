...
const STEPS = [
  ...
  { key: 'compute', label: 'Cartographie mondiale en cours' },
  { key: 'ai', label: 'Enrichissement des analyses par Soléna' },
  { key: 'pdf', label: 'Génération de ton PDF (18 pages)' },
  { key: 'email', label: 'Envoi par email' },
];

const AstrocartographieSucces = () => {
  ...
  const poll = useCallback(async () => {
    if (!sessionId) return;
    try {
      const r = await axios.get(`${API}/api/astrocartographie/status?session_id=${sessionId}`);
      setStatus(r.data || {});
  ...
  const stepState = (key) => {
    ...
    if (key === 'ai') return status.pdf_ready ? 'done' : (paid ? 'active' : 'pending');
  ...
  return (
    <div className="min-h-screen relative" style={{ padding: '110px 20px 140px' }} data-testid="astrocartographie-success-page">
      <SEO path="/astrocartographie/succes" title="Ton Astrocartographie est en cours · Plume Astrale" description="Ton rapport de 18 pages est en génération." />
      <div className="max-w-2xl mx-auto">
        <div className="text-center mb-10">
          ...
          <h1 style={{ fontFamily: 'Cormorant Garamond, serif', fontWeight: 300, lineHeight: 1.1, fontSize: 'clamp(32px, 5vw, 52px)', color: '#F5EEE0', marginBottom: 16 }}>
            Ta <em style={{ color: '#D4AF37', fontStyle: 'italic' }}>carte du monde</em><br />se dessine
          </h1>
          <p className="text-base max-w-lg mx-auto" style={{ color: 'rgba(227,215,255,0.75)', fontFamily: 'Cormorant Garamond, serif', fontStyle: 'italic', lineHeight: 1.6 }}>
            La génération prend 3 à 5 minutes (les analyses IA sont soignées).
            Ton PDF arrivera aussi par email.
          </p>
        </div>
        ...
        <div className="plume-glass p-8 mb-8">
          <ul className="space-y-4">
            {STEPS.map((s) => {
              ...
              return (
                <li key={s.key} className="flex items-center gap-3" data-testid={`astrocarto-step-${s.key}-${st}`}>
        ...
        <div className="text-center">
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            {status.pdf_url && (
              <a href={`${API}${status.pdf_url}`} target="_blank" rel="noopener noreferrer" className="plume-btn-primary" data-testid="astrocarto-download-btn">
                <Download className="w-4 h-4" strokeWidth={1.5} />
                Télécharger mon PDF
              </a>
            )}
            <Link to="/" className="plume-btn-secondary" data-testid="astrocarto-home-btn">
              Retour à l&apos;accueil
...
export default AstrocartographieSucces;
