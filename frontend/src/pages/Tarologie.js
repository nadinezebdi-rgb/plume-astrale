    ...
    <div className={`relative group ${isLocked ? 'overflow-hidden' : ''}`} data-testid={`carte-croix-${item.position_id}`}>
      ...
      <div className="bg-[#141418]/80 border border-[#C4A882]/20 rounded-sm p-3 hover:border-[#C4A882]/50 transition-all duration-300">
        ...
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-[#C4A882] text-xs uppercase tracking-widest font-medium">{posInfo.label}</span>
        </div>
        {/* Card image */}
        <div className="w-full aspect-[2/3] rounded-lg overflow-hidden mb-3 border border-[#C4A882]/30">
          {item.carte.image ? (
            <img src={`${API_URL}${item.carte.image}`} alt={item.carte.nom} className="w-full h-full object-cover" />
          ) : (
            <div className="w-full h-full flex items-center justify-center bg-gradient-to-b from-[#141418] to-[#1A1A20]">
              <span className="text-[#C4A882] text-2xl font-bold" style={{ fontFamily: 'Cormorant Garamond, serif' }}>{item.carte.numero}</span>
            </div>
          )}
        </div>
        {/* Card name */}
        <h4 className="text-[#E8E4DD] text-sm font-semibold text-center mb-1" style={{ fontFamily: 'Cormorant Garamond, serif' }}>{item.carte.nom}</h4>
        <p className="text-[#C4A882]/60 text-xs text-center">{item.carte.mots_cles || item.carte.energie}</p>
      </div>
    </div>
  );
};
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="bg-[rgba(255,255,255,0.02)] border border-[#C4A882]/20 rounded-sm p-6 mb-8" data-testid="offer-details">
            ...
            <div className="grid md:grid-cols-2 gap-3">
              {[
                ...
                <div key={i} className="flex items-center gap-2 text-[#A9A5A0]/70 text-sm">
                  <Star className="w-4 h-4 text-[#C4A882] flex-shrink-0" strokeWidth={1.5} />
                  {item}
                </div>
              ))}
            </div>
            <div className="mt-6 pt-4 border-t border-[#C4A882]/20 text-center">
              <span className="text-3xl font-bold text-gold-gradient" style={{ fontFamily: 'Cormorant Garamond, serif' }}>35 EUR</span>
              <p className="text-[#A9A5A0]/50 text-sm mt-1">Acces immediat + PDF telecharger</p>
            </div>
          </div>
          ...
          <div className="bg-[rgba(255,255,255,0.02)] border border-[#C4A882]/20 rounded-sm p-6 mb-8" data-testid="tarologie-form">
            ...
            <div className="grid md:grid-cols-2 gap-4 mb-4">
              <div>
                <label className="block text-[#C4A882] text-xs uppercase tracking-widest mb-2">Prenom</label>
                <input
                  type="text"
                  value={prenom}
              ...
              <div>
                <label className="block text-[#C4A882] text-xs uppercase tracking-widest mb-2">Date de naissance</label>
                <input
                  type="date"
                  value={dateNaissance}
          ...
          {tirage && (
            <div className="space-y-8 animate-fade-in" data-testid="tirage-en-croix">
              <div className="text-center mb-6">
                <h3 className="text-xl md:text-2xl" style={{ fontFamily: 'Cormorant Garamond, serif', color: '#E8E4DD' }}>
                  Votre Tirage en Croix
                </h3>
                <p className="text-[#A9A5A0]/40 text-sm mt-2">Cliquez sur une carte pour lire son interpretation</p>
              </div>
              ...
              <div className="relative mx-auto" style={{ maxWidth: '520px' }}>
                ...
                <svg className="absolute inset-0 w-full h-full pointer-events-none z-0 opacity-20" style={{ top: 0, left: 0 }}>
                  <line x1="50%" y1="28%" x2="50%" y2="38%" stroke="#C4A882" strokeWidth="1" strokeDasharray="4,4" />
                  <line x1="50%" y1="62%" x2="50%" y2="72%" stroke="#C4A882" strokeWidth="1" strokeDasharray="4,4" />
                  <line x1="30%" y1="50%" x2="37%" y2="50%" stroke="#C4A882" strokeWidth="1" strokeDasharray="4,4" />
                  <line x1="63%" y1="50%" x2="70%" y2="50%" stroke="#C4A882" strokeWidth="1" strokeDasharray="4,4" />
                </svg>
              </div>
              ...
              {getCard('centre') && (
                <div className="bg-[#141418]/80 border border-[#C4A882]/30 rounded-sm p-6" data-testid="interpretation-centre">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      <h4 className="text-[#E8E4DD] font-medium" style={{ fontFamily: 'Cormorant Garamond, serif' }}>{getCard('centre').carte.nom}</h4>
                      <p className="text-[#C4A882]/60 text-xs">Position Centre — La Situation Presente</p>
                    </div>
                  </div>
                  <p className="text-[#C4A882]/70 text-xs italic mb-3">{getCard('centre').carte.description_arcane}</p>
                  <p className="text-[#A9A5A0]/80 text-sm leading-relaxed">{getCard('centre').interpretation}</p>
                </div>
              )}
                ...
                <div className="bg-[#141418]/80 border border-[#C4A882]/20 rounded-sm p-6 relative overflow-hidden" data-testid="interpretations-locked">
                  <div className="absolute inset-0 bg-[#0B0B0F]/80 backdrop-blur-sm z-10 flex items-center justify-center">
                    <div className="text-center px-6">
                      ...
                      <h3 className="text-[#E8E4DD] text-lg mb-2" style={{ fontFamily: 'Cormorant Garamond, serif' }}>
                        4 Interpretations + Lecture Mediumnique
                      </h3>
                      <p className="text-[#A9A5A0]/60 text-sm mb-4">
                  ...
                  <div className="opacity-20 space-y-4">
                    {['obstacle', 'conseil', 'futur', 'synthese'].map(posId => {
                      ...
                      if (!card) return null;
                      return (
                        <div key={posId} className="border-b border-[#C4A882]/10 pb-3">
                          <h4 className="text-[#C4A882]">{card.position_nom}</h4>
                          <p className="text-[#A9A5A0]/50 text-sm">Lorem ipsum dolor sit amet consectetur adipiscing elit...</p>
                        </div>
                      );
                    })}
                  </div>
                    ...
                    return (
                      <div key={posId} className="bg-[#141418]/80 border border-[#C4A882]/20 rounded-sm p-6" data-testid={`interpretation-${posId}`}>
                        <div className="flex items-center gap-3 mb-4">
                          ...
                          <div>
                            <h4 className="text-[#E8E4DD] font-medium" style={{ fontFamily: 'Cormorant Garamond, serif' }}>{card.carte.nom}</h4>
                            <p className="text-[#C4A882]/60 text-xs">Position {posInfo.icon} — {posInfo.label}</p>
                          </div>
                        </div>
                        <p className="text-[#C4A882]/70 text-xs italic mb-3">{card.carte.description_arcane}</p>
                        <p className="text-[#A9A5A0]/80 text-sm leading-relaxed">{card.interpretation}</p>
                      </div>
                    );
                  })}
                </div>
                ...
                <div className="bg-[#141418]/80 border border-[#C4A882]/20 rounded-sm p-6 relative overflow-hidden" data-testid="lecture-locked">
                  <div className="absolute inset-0 bg-[#0B0B0F]/80 backdrop-blur-sm z-10 flex items-center justify-center">
                    <div className="text-center px-6">
                      ...
                      <p className="text-[#C4A882]/70 text-sm">Lecture mediumnique incluse</p>
                    </div>
                  </div>
                  <div className="opacity-20 p-4">
                    <h3 className="text-xl mb-4" style={{ fontFamily: 'Cormorant Garamond, serif', color: '#E8E4DD' }}>Lecture Mediumnique</h3>
                    <div className="space-y-3">
                      <div><h4 className="text-[#C4A882]">Empreinte du Passe</h4><p className="text-[#A9A5A0]/50">Lorem ipsum dolor sit amet...</p></div>
                      <div><h4 className="text-[#C4A882]">Energies du Present</h4><p className="text-[#A9A5A0]/50">Lorem ipsum dolor sit amet...</p></div>
                    </div>
                  </div>
                </div>
              )}
              ...
              {hasPaid && tirage.lecture_mediumnique && (
                <div className="bg-[#141418]/80 border border-[#C4A882]/20 rounded-sm p-6" data-testid="lecture-full">
                  ...
                  {[
                    ...
                    <div key={key} className="mb-4 pb-4 border-b border-[#C4A882]/10 last:border-0">
                      <h4 className="text-[#C4A882] font-medium mb-2">{label}</h4>
                      <p className="text-[#A9A5A0]/70 font-light leading-relaxed">
