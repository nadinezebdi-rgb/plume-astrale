...
const KarmaDestinWaiting = () => {
  ...
  useEffect(() => {
    ...
    const pollStatus = async () => {
      try {
        const response = await fetch(`${API_URL}/api/karma-destin/status?session_id=${sessionId}`);
        ...
        {status === 'loading' || status === 'waiting' ? (
          <>
            ...
            <h1 className="text-4xl font-bold text-[#D4AF37] mb-4">
              ✦ Analyse Karmique en cours ✦
            </h1>
            <p className="text-[#E3D7FF] mb-4 text-lg">
              Nous déchiffrons les leçons de ton âme...
            </p>
            <p className="text-[#9089B5]">
              Cela prend environ 30-60 secondes. Un email avec ton PDF arrive très bientôt!
            </p>
          </>
        ) : status === 'success' ? (
          <>
            ...
            <button
              onClick={() => navigate('/karma-destin-pdf')}
          ...
          <>
            ...
            <p className="text-[#E3D7FF] mb-8">
              {error || 'Impossible de générer ton analyse. Réessaye plus tard.'}
            </p>
            <button
              onClick={() => navigate('/karma-destin-pdf')}
...
export default KarmaDestinWaiting;
