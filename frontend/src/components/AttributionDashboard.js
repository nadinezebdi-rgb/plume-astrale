      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        <div>
          <div className="flex items-center gap-2">
            <TrendingUp className="w-4 h-4" style={{ color: '#D4AF37' }} strokeWidth={1.5} />
            <h2 className="text-lg" style={{ fontFamily: 'Cormorant Garamond, serif', color: '#F5EEE0', fontWeight: 400 }}>
              KPI Attribution
            </h2>
          </div>
          <p className="text-[11px] opacity-60 mt-1" style={{ color: '#F5EEE0' }}>
        ...
        <div className="flex gap-1.5">
          {RANGES.map(r => (
            <button key={r.key} onClick={() => setDays(r.key)}
              ...
              style={{
                border: '1px solid',
                borderColor: days === r.key ? '#D4AF37' : 'rgba(212,175,55,0.25)',
                color: days === r.key ? '#111625' : '#D4AF37',
                background: days === r.key ? '#D4AF37' : 'transparent',
          ...
          <button onClick={fetchData} disabled={loading}
            ...
            style={{ border: '1px solid rgba(212,175,55,0.25)', color: '#D4AF37', letterSpacing: '0.12em' }}
            ...
            <Section title="Par campagne" empty="Aucune campagne trackée pour l'instant.">
              {(data.by_campaign || []).length > 0 && (
                <table className="w-full text-xs" data-testid="table-campaigns">
                  <thead>
                    <tr style={{ color: '#D4AF37', opacity: 0.7 }}>
                  ...
                  <tbody>
                    {data.by_campaign.map((c, i) => (
                      <tr key={i} style={{ borderTop: '1px solid rgba(212,175,55,0.10)' }}
                          data-testid={`row-campaign-${i}`}>
                        <Td>
                          <span style={{ color: '#F5EEE0' }}>{c.source || 'direct'}</span>
                          <span className="opacity-50 mx-1">/</span>
                          <span style={{ color: '#D4AF37' }}>{c.campaign || 'unknown'}</span>
                        </Td>
                        <Td right>{c.leads}</Td>
                  ...
                  <tbody>
                    {data.by_source.map((s, i) => (
                      <tr key={i} style={{ borderTop: '1px solid rgba(212,175,55,0.10)' }}
                        ...
                        <Td>
                          <span style={{ color: '#F5EEE0', textTransform: 'capitalize' }}>{s.source}</span>
                        </Td>
                        <Td right>{s.leads}</Td>
                ...
                <tbody>
                  {data.by_content.map((c, i) => (
                    <tr key={i} style={{ borderTop: '1px solid rgba(212,175,55,0.10)' }}
                      ...
                      <Td><span style={{ color: '#F5EEE0', fontFamily: 'monospace' }}>{c.content}</span></Td>
                  ...
                  <tbody>
                    {data.recent_purchases.map((r, i) => (
                      <tr key={i} style={{ borderTop: '1px solid rgba(212,175,55,0.10)' }}
                        ...
                        <Td><span style={{ color: '#F5EEE0' }}>{r.email || '—'}</span></Td>
...
function KpiCard({ icon: Icon, label, value, accent, testid }) {
  return (
    <div className="rounded-xl p-4"
      style={{ background: accent ? 'rgba(212,175,55,0.10)' : 'rgba(0,0,0,0.20)', border: '1px solid rgba(212,175,55,0.20)' }}
      data-testid={testid}>
      <div className="flex items-center gap-1.5 mb-2 opacity-80">
        <Icon className="w-3.5 h-3.5" style={{ color: '#D4AF37' }} strokeWidth={1.5} />
        <span className="text-[10px] uppercase" style={{ color: '#D4AF37', letterSpacing: '0.2em' }}>{label}</span>
      </div>
      <div className="text-2xl" style={{ fontFamily: 'Cormorant Garamond, serif', color: accent ? '#D4AF37' : '#F5EEE0', fontWeight: 400 }}>
        {value}
      </div>
    </div>
  );
...
function Section({ title, children, empty }) {
  ...
  return (
    <div>
      <div className="text-[10px] uppercase mb-3" style={{ color: '#D4AF37', letterSpacing: '0.25em' }}>{title}</div>
      {isEmpty ? (
        <div className="rounded-lg p-4 text-xs opacity-60 text-center" style={{ background: 'rgba(0,0,0,0.20)', border: '1px dashed rgba(212,175,55,0.20)' }}>
          {empty}
        </div>
      ) : children}
    </div>
...
function Td({ children, right, accent }) {
  return <td className={`py-2 ${right ? 'text-right' : 'text-left'}`} style={{ color: accent ? '#D4AF37' : undefined, fontWeight: accent ? 500 : 400 }}>{children}</td>;
}
