...
- **Service `services/edition_reliee_service.py`** :
  - `create_edition_reliee_checkout(...)` : session Stripe (14 900 cents) avec `metadata.kind='edition_reliee'`, écriture `payment_transactions` en `initiated/unpaid` avec métadata riche (pdf_ctx, purchaser info, recipient info, dédicace 800 chars, ville de naissance).
  - `handle_edition_reliee_webhook(session_id)` : marque `completed/paid` → génère le PDF 49 pages en réutilisant `handle_theme_natal_oneshot_webhook` (patch temporaire `kind` pour piloter la génération puis restauration) → appelle `create_print_approval()` qui envoie l'email 72h + trace `print_approval_id` + `deadline_at` dans le tx metadata. Idempotent (skip si pdf déjà généré ou print_approval déjà créé pour ce session_id). Soft-fail actionnable si la table `print_approvals` n'existe pas.
- **Route `routes/edition_reliee.py`** :
  - `POST /api/edition-reliee/checkout` : Pydantic validation stricte (email format, `birth_date` YYYY-MM-DD, `birth_time` HH:MM, `birth_city` min 2 chars, `dedication` max 800). Retourne `{url, session_id}`.
  - `GET /api/edition-reliee/status?session_id=…` : polling live pour la page merci (renvoie `pdf_ready`, `email_sent`, `deadline_at`).
- **Webhook dispatcher `server.py`** : ajout de la branche `md.get('kind') == 'edition_reliee'` juste après gift_card, appelle `handle_edition_reliee_webhook(session_id)`.
- **Frontend `/edition-reliee`** : les CTA "Composer son livre — 149 €" (top + bottom) ne redirigent plus vers `/carte-cadeau`, ils ouvrent une **modal de checkout inline** (`CheckoutModal`) avec 8 champs (email/prénom acheteur + prénom/date/heure/ville destinataire + dédicace optionnelle) et bouton "Payer 149 € en toute sécurité". La modal poste vers `/api/edition-reliee/checkout` puis redirige vers Stripe. Le lien `/carte-cadeau` reste dans le body ("Vous n'avez pas son heure de naissance ?") comme escape valve. data-testids `er-checkout-modal`, `er-form-{email,purchaser,recipient,birth-date,birth-time,birth-city,dedication,submit}`.
- **Frontend `/edition-reliee/merci`** (nouveau `EditionRelieeMerci.js`) : page post-paiement qui poll `/api/edition-reliee/status` toutes les 4 s (30 tentatives max, arrête dès `email_sent`). Trois puces animées (Paiement reçu / Composition du livre / Email de relecture envoyé) avec halos or/vert selon état. Explique le flow 72h. Signée "— Nadine". noindex.
- **Tests pytest `tests/test_edition_reliee.py`** — 9/9 pass : PACKS config, validation Pydantic (422 sur email/date/heure invalides), status 404, webhook dispatcher patched, service API, routes App.js présentes, CTAs ouvrent la modal (regression : les CTAs `er-cta-primary-*` ne DOIVENT PLUS être des `<a href="/carte-cadeau">`).

**Statut end-to-end** : le flow complet est actif en mode Stripe LIVE. Une commande réelle 149 € génèrera le PDF, créera un `print_approval` et enverra l'email avec liens d'approbation 72h dès que l'utilisateur aura joué la migration SQL `2026_02_print_approvals.sql` dans Supabase.
...
- **QR Referral Tracker** : nouveau `services/qr_referral_tracker.py` + route publique `/r/{code}` (non-préfixée `/api` car doit tenir dans le QR imprimé). Log Supabase best-effort (tables `referral_scans` + `referral_scan_counters`), cookie `ref` 30 jours, redirection vers `/?ref={code}&utm_source=pdf_qr&utm_medium=print&utm_campaign=colophon`. Le QR embed maintenant `/r/{code}` (au lieu de `/?ref=`) pour tracker les scans réels vs impressions PDF. Endpoint admin lecture : `GET /api/admin/referral-scan-stats?top=50`.
- **Print-ready BleedBox/TrimBox** : `pdf_luxury_helpers.add_print_boxes(pdf_bytes, bleed_mm=3.0)` post-processeur pypdf qui injecte BleedBox (= MediaBox) et TrimBox (MediaBox − 3mm de chaque côté) sur chaque page. Le PDF devient acceptable par imprimeurs pro (workflow standard offset). Note : embed ICC CMYK non fait (ReportLab reste RGB) — conversion RGB→CMYK à faire par l'imprimeur (Ghostscript/InDesign). Tests verrouillent la présence des boîtes + safe fallback sur input invalide.

### Tokens de téléchargement · Pagination · Colophon · Navbar (2026-02-26)
- **SEC — Migration Voyage Karmique + Synastrie vers tokens opaques** : `services/pdf_download.py` étendu avec `_MULTI_PDF_PRODUCTS` (Voyage Karmique = 2 PDFs par session) + fallback `_download_synastrie_pdf()` (table séparée `synastrie_purchases`, token extrait du query string de `pdf_path` — pas de migration schema requise). `server.py` : suppression des mounts statiques `/api/assets/synastrie/*` et `/api/assets/voyage_karmique/*`. Compat legacy : les anciens `pdf_path=/api/assets/...` restent servis via session_id (surface d'attaque identique à l'ancien flux). Test suite mise à jour (10/10 pass).
- **Colophon Nocturne** : nouveau `services/pdf_colophon.py` (fallback safe si `qrcode` absent) — citation Soléna + signature « voix éditoriale » + bloc partenariat -15% avec QR code doré (Cinzel) pointant vers `plume-astrale.fr/?ref={code}` + kicker éditorial. Wired dans Karma Destin, Numérologie, Voyage Karmique via kwarg `referral_code`+`referral_link`. Dépendance ajoutée : `qrcode==8.2`.
- **Pagination promise tenue** : Karma & Destin passe de 16 → **24 pages** (chap. VIII Affirmations, IX Journal karmique, X Feuille de route 90j + split Nœud Nord/Sud). Numérologie passe de 8 → **21 pages** (chap. VII Compatibilités numériques, VIII Affirmations, IX Journal + un nombre-clé par page). Tests pytest verrouillent les seuils ≥22 / ≥16 pages.
- **Navbar** : lien "Nos livres" ajouté dans `NavbarV2.NAV_LINKS` (retiré du footer, où il faisait doublon avec /livres = Services).
